...
int main (int argc, char** argv)
{
	...
	//save to file
	std::string ground_coeff_file = std::string("ground_coeff.txt");
	ros::param::get("~ground_coeff_file", ground_coeff_file);
	std::cout << ground_coeff_file << std::endl;
	std::string file_location = ros::package::getPath("pcl_detection")+"/data/"+ground_coeff_file;
	write_vector_to_file(file_location.c_str(),ground_coeffs,4);
	ROS_INFO("Written to file!");
